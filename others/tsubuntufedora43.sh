#!/bin/bash
echo "┌──────────────────────────────────────────┐"
echo "│         Tsubuntu for Fedora 43           │"
echo "└──────────────────────────────────────────┘"
# Fedora does not support gnome-software-plugin-snap, giving option to install App Centre instead
sudo dnf install gnome-tweaks pkexec notify-send unzip gnome-extensions-app dconf-editor snapd firewall-config bash-completion fuse-libs -y
...
gsettings set org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/custom1/ command 'ptyxis'
